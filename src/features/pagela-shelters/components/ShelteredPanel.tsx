...
export function ShelteredPanel({
  ...
  onSelect: (c: ShelteredResponseDto) => void;
  ...
  const sort = useMemo<ShelteredSort>(() => ({ id: "updatedAt", desc: true }), []);
  const filters: ShelteredFilters = useMemo(
    () => ({
      searchString: dq || undefined,
      shelterNumber: shelter?.number ?? undefined,
    }),
    [dq, shelter?.number]
  );
  ...
  const fetchShelteredren = useCallback(async () => {
    if (!shelter) {
      setRows([]);
      setTotal(0);
      return;
    ...
    const key = JSON.stringify({ page, limit, filters, sort, shelterId: shelter.id });
    ...
    setError("");
    try {
      const data = await apiFetchShelteredren({ page, limit, filters, sort });
  ...
  }, [shelter, page, limit, filters, sort]);
    ...
    <Stack sx={{ height: "100%" }} spacing={2}>
      ...
      <Stack direction="row" alignItems="center" justifyContent="space-between">
        <Tooltip title="Recarregar">
          <IconButton onClick={() => fetchShelteredren()}>
            <RefreshIcon />
          </IconButton>
        </Tooltip>
        <Pagination
